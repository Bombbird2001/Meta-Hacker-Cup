...
def build_adjacency_matrix(tunnels: List[tuple[int, int]], caves: int) -> List[List[int]]:
    matrix = [[0 for _ in range(caves)] for _ in range(caves)]
...
with open("C1_output.txt", "w+") as of:
    with open("gold_mine_chapter_1_input.txt", "r") as f:
        mines = int(f.readline().strip())
        for i in range(mines):
            caves = int(f.readline().strip())
            tunnel_list = []
            ...
            max_ore_count = max_ore(adj_matrix, gold_list, 1)
            if i > 0:
                of.write("\n")
